...
const Body: React.FC<BodyProps> = ({handleClickDropbox, handleClickGoogle, item, index}) => {
    ...
    return (
        ...
        <tr key={index} onMouseEnter={()=>{setId(item.id)}} onMouseLeave={()=>{
            setId("")
            ...
            <td className={styles.listItem}>
                {item[".tag"] === "folder" || item["mimeType"] ==="application/vnd.google-apps.folder" ? (
                <a  href={isGoogle ? `/file${item.id}` : `/file${item.path_display}`}
                onClick={(e) => onClickButton(e)}>
                    <span
                    className={`icon-folder-open ${styles.icon}`}
                    />
                ...
                <div>
                    <span
                    className={`icon-file ${styles.icon}`}
                    />
                    {isGoogle ? item.title : item.name}
                </div>
                )}
            ...
            <td> {isGoogle ? item["client_modified"] ? item["client_modified"].replace(/[\\T\\Z]/g, " ") : "--" : item["modifiedDate"] ? item["modifiedDate"].replace(/[\\T\\Z]/g, " ") : "--"}  </td>
