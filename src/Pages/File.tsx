...
import { APIContext } from "../APIrequest";
  ...
  const File: React.FC = () => {
        ...
        const { Authorization, getListFolderDropbox, getListFolderGoogle, getfolder } = useContext(APIContext)
          ...
          if (!localStorage.getItem("token")) {
            if (!window.location.hash) { 
              ...
              .then(() =>{window.location.reload()})
                ...
                localStorage.setItem("isAuth", "true" )
                localStorage.setItem("token", new URLSearchParams(window.location.hash).get("access_token")!);
        ...
        const handleClickGoogle = (e: React.MouseEvent<HTMLAnchorElement>, path: string, name?: string) =>{
          e.preventDefault()
        ...
        return (
          <>
          <Toolbar/>
          <Breadcrumb pathFolder={pathFolder} pathBack={pathBack} handleClickDropbox={handleClickDropbox} handleClickBackGoogle={handleClickBackGoogle} id={id}/>
